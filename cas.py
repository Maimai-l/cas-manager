...
# *static* baseline is owned by cas_prompts._DEFAULTS["reflection.system"];
# the per-experience context (proposal text + LOs + strand) is appended below
# at runtime — that context is dynamic and never user-editable.
import cas_prompts


def _ai_system_with_context(proposal: Optional[dict]) -> str:
    ...
    # `strand` may now hold multiple comma-separated strands (e.g.
...
def _final_system_with_context(proposal: Optional[dict]) -> str:
...
def _format_history_block(history: Optional[list]) -> str:
...
def _build_prompt(notes: str, system: str, kind: str = "reflection",
...
def ai_generate(notes: str, model: str,
    ...
    if provider == "ollama":
        ...
        return _ai_via_ollama(system, user, ollama_model, ollama_url)
